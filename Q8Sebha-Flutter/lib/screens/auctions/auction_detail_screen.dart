  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        boxShadow: [BoxShadow(color: Color(0xFF1A1A2E),
            blurRadius: 12, offset: const Offset(0, 4))],
      ),
      child: isLoading
          ? const Center(child: SizedBox(width: 22, height: 22,
        ...
        Expanded(
          child: OutlinedButton.icon(
            ...
            label: const Text('إبلاغ',
              style: TextStyle(fontFamily: 'Tajawal', color: Colors.red, fontSize: 13)),
          ),
        ),
        const SizedBox(width: 10),
        Expanded(
...
class _RatingSectionState extends State<_RatingSection> {
  ...
  String? _error;
  List<dynamic> _reviews = [];
  double? _avgRating;   // null = لا تقييمات بعد
  int _reviewCount = 0;
  bool _reviewsLoading = true;
          ...
          const Align(alignment: Alignment.centerRight,
            child: Text('تقييمات البائع',
              style: TextStyle(fontFamily: 'Tajawal', fontWeight: FontWeight.w700,
                  fontSize: 13, color: AppTheme.textMid))),
          const SizedBox(height: 8),
          ...(_reviews.take(3).map((r) => _ReviewItem(review: Map<String,dynamic>.from(r)))),
        ],
      ]),
    );
  }
    ...
    if (bids.isEmpty) {
      return Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFF7F6F3),
            borderRadius: BorderRadius.circular(14)),
        child: const Center(child: Text('لا توجد مزايدات بعد',
            style: TextStyle(fontFamily: 'Tajawal', fontSize: 13, color: AppTheme.textLight))),
      );
    }
    final shown = showAll ? bids : bids.take(5).toList();
        ...
        ...shown.asMap().entries.map((e) {
          ...
          return Container(
            ...
            child: Row(children: [
              Text('$amount د.ك',
                style: TextStyle(fontFamily: 'Tajawal', fontWeight: FontWeight.w700,
                    fontSize: 13, color: i == 0 ? AppTheme.gold : AppTheme.textDark)),
              const Spacer(),
              if (i == 0) const Icon(Icons.emoji_events, color: AppTheme.gold, size: 14),
              const SizedBox(width: 4),
              Text(name, style: const TextStyle(fontFamily: 'Tajawal',
